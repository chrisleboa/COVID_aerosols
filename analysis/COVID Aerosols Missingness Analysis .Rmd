...
```{r}
merged_data %>%
  mutate(across(2:last_col(), is.na)) %>%  # replace all NA with TRUE and else FALSE
  pivot_longer(-sampleid, names_to = "var") %>%  # pivot longer
  filter(value) %>%   # remove the FALSE rows
  group_by(sampleid) %>%    # group by the ID
  summarise(`Missing Variables` = toString(var)) %>% 
  write_csv("missingness.csv")# convert the variable names to a string column
